...
function updateISTClock() {
    ...
    istTimeDisplay.textContent = `${hours}:${minutes}:${seconds}`;
...
function init() {
    ...
    const kolkataTz = timeZones.find(tz => tz.location.includes('Kolkata'));
    if (kolkataTz) {
        highlightTimezone(kolkataTz);
    } else if (timeZones.length > 0) {
        highlightTimezone(timeZones[15]);
    }
}
